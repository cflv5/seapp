	...
	<div class="grid">
		<div class="col-12">
			<div class="card">
				...
				<DataTable ref="dt" :value="files" v-model:selection="selectedProducts" dataKey="id" :paginator="true"
					:rows="10" :filters="filters"
					paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
					:rowsPerPageOptions="[5, 10, 25]"
					currentPageReportTemplate="Showing {first} to {last} of {totalRecords} files"
					responsiveLayout="scroll" class="p-datatable-md" :stripedRows="true">
					<template #header>
					...
					<Column field="accessType" header="Access Type" :sortable="true"
					...
					<Column header="Show">
						<template #body="slotProps">
							<Button icon="pi pi-eye" class="p-button-link p-button-success mr-2"
								@click="showFile(slotProps.data)" />
						</template>
					</Column>
				</DataTable>

				<Dialog v-model:visible="fileDialog" :style="{ width: '450px' }" header="Product Details"
					:modal="true" class="p-fluid">
					<div class="field">
						<label for="inventoryStatus" class="mb-3">Access Type</label>
						<Dropdown id="inventoryStatus" v-model="file.accessType" :options="statuses"
					...
					<template #footer>
						<Button label="Cancel" icon="pi pi-times" class="p-button-text" @click="fileDialog = false" />
...
export default {
	...
	async mounted() {
		...
		try {
			...
			console.log(e);
			this.$toast.add({ severity: 'error', summary: 'Warning', detail: 'An error occured. Please reload the page', life: 3000 });
			return;
		}
